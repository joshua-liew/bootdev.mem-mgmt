...
bool snek_array_set(snek_object_t *snek_obj, size_t index, snek_object_t *value) {
  ...
  // Check if there already was an object at the index
  snek_object_t *prev_val = snek_array_get(snek_obj, index);

  snek_obj->data.v_array.elements[index] = value;
  snek_object_t *index_ptr = snek_obj->data.v_array.elements[index];
  refcount_inc(index_ptr);

  // If already stored object decrement
  if (prev_val != NULL) {
    refcount_dec(prev_val);
  }
